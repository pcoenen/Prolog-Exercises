...
connect(Grid,SortedLinks,UsedPositions,Solution) :-
    find_paths(Grid,SortedLinks,UsedPositions,Solution).  

find_paths(grid(X,Y),[],UP,[]):-
    list_to_set(UP,Set),
    length(Set,Length),
    Length =:= X * Y.

find_paths(Grid,[link(V, P1, P2)|R],UsedPositions,[connects(V,[P1|S])|Sol]):-
    find_path(Grid,P1,P2,UsedPositions,S),
    append(S,UsedPositions,NU),
    find_paths(Grid,R,NU,Sol).

find_path(_Grid,P,P,_Used,[]).

find_path(Grid,pos(X1,Y1), P2,Used,[pos(XN,Y1)|Sol]):-
    (
        XN is X1 + 1
    ;
        XN is X1 - 1
    ),
    is_valid_position(Grid,pos(XN,Y1)),
    (
        \+ member(pos(XN,Y1),Used)
    ;
        P2 == pos(XN,Y1)
    ),
    find_path(Grid,pos(XN,Y1),P2,[pos(XN,Y1)|Used],Sol).

find_path(Grid,pos(X1,Y1), P2,Used,[pos(X1,YN)|Sol]):-
    (
        YN is Y1 + 1
    ;
        YN is Y1 - 1
    ),
    is_valid_position(Grid,pos(X1,YN)),
    (
        \+ member(pos(X1,YN),Used)
    ;
        P2 == pos(X1,YN)
    ),
    find_path(Grid,pos(X1,YN),P2,[pos(X1,YN)|Used],Sol).
